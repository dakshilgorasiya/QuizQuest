...
export default function PastQuiz() {
  ...
  useEffect(() => {
    if (!accessToken) {
      ...
      localStorage.setItem("url", "/past-quiz");
      alert("Kindly login to view history");
      navigate("/login");
    }
  }, []);
  ...
  useEffect(() => {
    ...
    if (!accessToken) {
      setLoading(false);
      setError("Kindly login to view quiz");
      return;
    }

    fetch(`${SERVER}/quiz/history`, {
      method: "GET",
      headers: {
